...
public class BurningShip extends FractalGenerator { // вычисление фрактала BurningShip
    ...
    public void getInitialRange(Rectangle2D.Double range) { // функция позволяет генератору фракталов определить
        ...
        range.y = -2.5;                                    // range - прямоугольный объект
    ...
    public int numIterations(double x, double y) {
        ...
        for(int i=0;i<MAX_ITERATIONS;i++){
            ...
            double y2=2*Math.abs(zr)*Math.abs(zi)+y;
            zr=x2;
            zi=y2;
            if (zr*zr+zi*zi>4){
                return i;
